...
bool drm_update_degamma_lut(struct drm_t *drm)
{
	...
	for ( int i = 0; i < lut_entries; i++ )
	{
		...
		degamma_lut[i].red   = safe_pow( input, drm->pending.color_degamma_exponent[0] );
		degamma_lut[i].green = safe_pow( input, drm->pending.color_degamma_exponent[1] );
		degamma_lut[i].blue  = safe_pow( input, drm->pending.color_degamma_exponent[2] );
	}
